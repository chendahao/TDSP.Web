    ...
    <v-container fluid>
      ...
      >
        <template v-slot:item.actions="{ item }">
          <v-tooltip bottom>
            <template v-slot:activator="{ on }">
              <v-icon
                color="info"
                v-on="on"
                @click="edit(item.customId,'detail')"
              >view_quilt</v-icon>
            </template>
            <span>详细</span>
          </v-tooltip>
          <!-- <v-tooltip bottom>
            <template v-slot:activator="{ on }">
              <v-icon color="primary" v-on="on" @click="edit(item.customId,'edit')">edit</v-icon>
            </template>
            <span>编辑</span>
          </v-tooltip>
          <v-tooltip bottom>
            <template v-slot:activator="{ on }">
              <v-icon
                color="warning"
                v-on="on"
                @click="deleteitem(item.name,item.customId)"
              >delete</v-icon>
            </template>
            <span>删除</span>
          </v-tooltip> -->
  ...
  data () {
    return {
      headers: [
        ...
        { text: '定线制宽度(海里)', sortable: false, value: 'width' },
        { text: '说明', sortable: false, value: 'description' },
