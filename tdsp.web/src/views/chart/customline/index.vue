<template>
  <div>
    <PageHeader headertitle="定线制水域船舶统计">
      <v-spacer></v-spacer>
    </PageHeader>
    <v-container fluid>
        ...
        <v-layout row wrap>
          ...
          <v-flex md2>
            <v-menu v-model="menu1" :close-on-content-click="false" :nudge-right="40" transition="scale-transition"
              ...
              <template v-slot:activator="{ on }">
                <v-text-field v-model="date1" placeholder="开始时间" hint="开始时间" persistent-hint append-icon="event" readonly v-on="on">
                </v-text-field>
              </template>
              <v-date-picker v-model="date1" @input="menu1 = false" locale="zh-cn" :dayFormat="dayformat">
              </v-date-picker>
            </v-menu>
          </v-flex>
          <v-flex md2 class="ml-3">
            <v-menu v-model="menu2" :close-on-content-click="false" :nudge-right="40" transition="scale-transition"
              offset-y min-width="290px">
              <template v-slot:activator="{ on }">
                <v-text-field v-model="date2" placeholder="截止时间" hint="截止时间" persistent-hint append-icon="dvr" readonly v-on="on">
                </v-text-field>
              </template>
              <v-date-picker v-model="date2" @input="menu2 = false" locale="zh-cn" :dayFormat="dayformat">
              </v-date-picker>
            </v-menu>
          </v-flex>
          <v-flex md3 class="ml-3">
            <v-select :items="items" item-text="name" item-value="customId" v-model="item"
              append-icon="format_list_bulleted" placeholder="定线制名称" hint="定线制名称" persistent-hint></v-select>
          </v-flex>
        </v-layout>
        <v-spacer></v-spacer>
...
export default {
  ...
  methods: {
    getitems: async function () {
      let {
        data
      } = await lineapi.GetAll()
      this.items = data.values
      this.item = this.items[0].customId
    },
    getchartdata: async function () {
      let item = await api.GetCustomLineFlowById(this.item, this.date1, this.date2)
      let data = item.data
